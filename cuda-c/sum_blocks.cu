...
__global__ void sum_kernel(const int *x,const int *y, int *res){
    ...
    // blockIdx is the entity that provides the index
    // of the block to which the kernel is assigned
    res[blockIdx.x] = x[blockIdx.x] + y[blockIdx.x];
...
int main(int argc, char **argv){
    ...
    sum_kernel<<<4,1>>>(dev_a, dev_b, dev_c);
